...
static void method_call_cb(FlMethodChannel* channel, FlMethodCall* method_call,
                           gpointer user_data) {
  FlutterVolumeControllerPlugin* plugin = FLUTTER_VOLUME_CONTROLLER_PLUGIN(user_data);
  flutter_volume_controller_plugin_handle_method_call(plugin, method_call);
}

void flutter_volume_controller_plugin_register_with_registrar(FlPluginRegistrar* registrar) {
  FlutterVolumeControllerPlugin* plugin = FLUTTER_VOLUME_CONTROLLER_PLUGIN(
